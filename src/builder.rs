...
impl<'ctx> Builder<'ctx> {
    ...
    pub fn build_memcpy(
        ...
        if !is_alignment_ok(src_align_bytes) {
            return Err(BuilderError::AlignmentError(AlignmentError::NonPowerOfTwo(src_align_bytes)));
        }

        if !is_alignment_ok(dest_align_bytes) {
            return Err(BuilderError::AlignmentError(AlignmentError::NonPowerOfTwo(dest_align_bytes)));
        }
    ...
    pub fn build_malloc<T: BasicType<'ctx>>(&self, ty: T, name: &str) -> Result<PointerValue<'ctx>, BuilderError> {
        ...
        if !ty.is_sized() {
            return Err(BuilderError::AlignmentError(AlignmentError::UnsizedType));
        }
